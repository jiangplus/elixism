# Real-package benchmarks across runtimes

Two real hex.pm packages run on the **same source** across the BEAM (real Elixir)
and Elixism (Guile), each with a correctness gate:

| benchmark | package | workload | runner |
|---|---|---|---|
| graph | [libgraph](https://github.com/bitwalker/libgraph) | build + SCC + acyclic? + reachable | `./run-graph.sh` |
| decimal | [Decimal](https://github.com/ericmj/decimal) | arbitrary-precision arithmetic (new/mult/sub/add fold) | `./run-decimal.sh` |

Both libraries run **unmodified** — every gap was closed in Elixism, not the
library (see git history / `.{libgraph,decimal}-commit`). Sample slowdowns
(Apple M-series, Guile 3.0.11): libgraph **~10–14× and flat** from N=100 to
1600 (the persistent HAMT map removed the old super-linear 46×→109× blowup —
maps were alist-backed); Decimal ~48× on a fold of N=3000 arithmetic ops
(arithmetic-bound, not map-bound, so unaffected by the HAMT).

For Decimal, the byte-level **string parser** (`Decimal.new("1.5")`) and `div`/
`compare` (which need a `pow10` table generated by a compile-time `Enum.reduce`)
don't run; the benchmark uses integer-constructed decimals and `new/mult/sub/add/
negate/abs/to_string`, which match BEAM exactly.
