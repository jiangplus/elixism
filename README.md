...
* **GenServer** — `start_link`/`call`/`cast`/`stop` with `init`,
  `handle_call`, `handle_cast`, `handle_info` callbacks and held state.

Not yet: binary pattern-matching, `Supervisor` trees, true pre-emption.
...
```sh
make test                      # run the full suite (189 tests)
...
test/            189 tests across lexer, parser, runtime, integration, process
...
189 tests: `test-lexer`, `test-parser`, `test-runtime` (value model),
