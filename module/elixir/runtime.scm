...
(define (ht-collision? x) (and (vector? x) (eq? (vector-ref x 0) 'collision)))
(define (ht-node-slots n) (cdr (vector->list n)))
(define (ht-make-node bitmap slots) (list->vector (cons bitmap slots)))
(define (ht-make-collision pairs) (list->vector (cons 'collision pairs)))
(define (ht-coll-pairs n) (cdr (vector->list n)))

(define (ht-list-insert lst i x) (append (list-head lst i) (list x) (list-tail lst i)))
(define (ht-list-set lst i x) (append (list-head lst i) (list x) (list-tail lst (+ i 1))))
(define (ht-list-remove lst i) (append (list-head lst i) (list-tail lst (+ i 1))))
(define (ht-slot-index h shift) (logand (ash h (- shift)) 31))

(define (hamt-get node k default)
  (let go ((node node) (h (ex-hash k)) (shift 0))
    (if (ht-collision? node)
        (let ((p (find (lambda (kv) (ex-equal? (car kv) k)) (ht-coll-pairs node))))
          (if p (cdr p) default))
        (let* ((bm (vector-ref node 0))
               (bit (ash 1 (ht-slot-index h shift))))
...
(define (hamt-put node k v)            ; -> (values node added?)
  (let go ((node node) (h (ex-hash k)) (shift 0))
    (if (ht-collision? node)
        (let* ((ps (ht-coll-pairs node))
               (had (find (lambda (kv) (ex-equal? (car kv) k)) ps)))
          (if had
              (values (ht-make-collision (map (lambda (kv) (if (ex-equal? (car kv) k) (cons k v) kv)) ps)) #f)
              (values (ht-make-collision (cons (cons k v) ps)) #t)))
        (let* ((bm (vector-ref node 0))
               (bit (ash 1 (ht-slot-index h shift)))
               (pos (logcount (logand bm (- bit 1))))
               (slots (ht-node-slots node)))
          (if (zero? (logand bm bit))
              (values (ht-make-node (logior bm bit) (ht-list-insert slots pos (cons k v))) #t)
              (let ((slot (list-ref slots pos)))
                (cond
                 ((vector? slot)
                  (call-with-values (lambda () (go slot h (+ shift 5)))
                    (lambda (ns added) (values (ht-make-node bm (ht-list-set slots pos ns)) added))))
                 ((ex-equal? (car slot) k)
                  (values (ht-make-node bm (ht-list-set slots pos (cons k v))) #f))
                 (else
                  (values (ht-make-node bm (ht-list-set slots pos
...
(define (hamt-merge l1 l2 h2 shift)    ; split two leaves into a sub-node
  (let ((h1 (ex-hash (car l1))))
    (let go ((h1 h1) (h2 h2) (shift shift))
      (if (>= shift 30)
          (ht-make-collision (list l1 l2))
          (let ((i1 (ht-slot-index h1 shift)) (i2 (ht-slot-index h2 shift)))
            (if (= i1 i2)
                (ht-make-node (ash 1 i1) (list (go h1 h2 (+ shift 5))))
                (let ((b1 (ash 1 i1)) (b2 (ash 1 i2)))
                  (if (< i1 i2)
                      (ht-make-node (logior b1 b2) (list l1 l2))
                      (ht-make-node (logior b1 b2) (list l2 l1))))))))))
    ...
    (if (ht-collision? node)
        (let ((ps (ht-coll-pairs node)))
          (if (find (lambda (kv) (ex-equal? (car kv) k)) ps)
              (values (ht-make-collision (filter (lambda (kv) (not (ex-equal? (car kv) k))) ps)) #t)
              (values node #f)))
        (let* ((bm (vector-ref node 0))
               (bit (ash 1 (ht-slot-index h shift)))
               (pos (logcount (logand bm (- bit 1))))
              ...
              (let ((slot (list-ref slots pos)))
                (cond
                 ((vector? slot)
                  (call-with-values (lambda () (go slot h (+ shift 5)))
                    (lambda (ns removed)
                      (if removed (values (ht-make-node bm (ht-list-set slots pos ns)) #t)
                          (values node #f)))))
                 ((ex-equal? (car slot) k)
                  (values (ht-make-node (logxor bm bit) (ht-list-remove slots pos)) #t))
                 (else (values node #f)))))))))

(define (hamt->alist node)
  (if (ht-collision? node) (ht-coll-pairs node)
